...
export interface FormField {
  name: string;
  type: 'text' | 'email' | 'password' | 'number' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'date' | 'url' | 'tel';
  label: string;
...
export class FormBuilderComponent implements OnInit {
  ...
  buildForm() {
    const formGroup: any = {};
    const urlRegex = /^(https?:\/\/)?([\w-]+(\.[\w-]+)+\/?|localhost(:\d+)?(\/.*)?)$/
    ...
    this.fields.forEach(field => {
      ...
      formGroup[field.name] = [field.value || (field.type === 'checkbox' ? false : ''), validators];
    });
  ...
  getErrorMessage(field: string, type: string): string {
    ...
    if (control?.hasError('pattern') && type === 'url') return 'Invalid url format'
    if (control?.hasError('pattern') && type === 'tel') return 'Invalid phone number format'
    return '';
  }
